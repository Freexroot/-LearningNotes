...
![BS结构系统的通信原理2](D:\大学资料\学习资料\Java后端\JavaWeb\tomcat&servlet\老杜2021最新JavaWeb\文档\BS结构系统的通信原理2.png)
...
![BS结构系统的角色和协议](D:\大学资料\学习资料\Java后端\JavaWeb\tomcat&servlet\老杜2021最新JavaWeb\文档\BS结构系统的角色和协议.png)
...
- 思考：我们自己new的Servlet对象受WEB容器的管理吗？
  ...
  - WEB容器创建的Servlet对象，这些Servlet对象都会被放到一个集合当中（HashMap），只有放到这个HashMap集合中的Servlet才能够被WEB容器管理，自己new的Servlet对象不会被WEB容器管理。（因为自己new的Servlet对象不在容器当中）
  - web容器底层应该有一个HashMap这样的集合，在这个集合当中存储了Servlet对象和请求路径之间的关系
  - ![WEB容器中的Map集合](D:\大学资料\学习资料\Java后端\JavaWeb\tomcat&servlet\老杜2021最新JavaWeb\文档\WEB容器中的Map集合.png)
...
- Servlet对象生命周期
  ...
  - 根据以上输出结果得知，用户在发送第二次，或者第三次，或者第四次请求的时候，Servlet对象并没有新建，还是使用之前创建好的Servlet对象，直接调用该Servlet对象的service方法，这说明：
    ...
    - 第二：“无参数构造”和“init”方法只在第一次用户发送请求的时候执行。也就是说无参数构造方法只执行一次。init方法也只被Tomcat服务器调用一次。
...
- 为什么要编写GenericServlet抽象类而不是使用使用Servlet接口？ 

  - 符合适配器设计模式Adapter。比如：手机直接插到220V的电压上，手机直接就报废了。怎么办？可以找一个充电器。这个充电器就是一个适配器。手机连接适配器。适配器连接220V的电压。这样问题就解决了。屏蔽Servlet少用的方法，扩展额外的方法,更简洁。
...
- 什么是ServletConfig？
  ...
  - ServletConfig对象中封装了<servlet></servlet>标签中的配置信息。（web.xml文件中servlet的配置信息）
    ...
    <servlet>
        ...
        <!--这里是可以配置一个Servlet对象的初始化信息（init）的。-->
        <init-param>
...
- ServletConfig接口有哪些常用的方法？
  ...
  - ```java
    ...
    public Enumeration<String> getInitParameterNames(); // 获取所有的初始化参数的name
    		...
    		//Enumeration<String> initParameterNames = this.getInitParameterNames(); //也行
            // 遍历集合
    ...
    public String getServletName(); // 获取<servlet-name>标签下的name
    ```
...
- 但是，只要在同一个webapp当中，只要在同一个应用当中，所有的Servlet对象都是共享同一个ServletContext对象的。
...
- ServletContext接口中有哪些常用的方法？
  ...
  - ```java
    // 获取应用的根文件名（非常重要），因为在java源代码当中有一些地方可能会需要应用的根文件名，这个方法可以动态获取应用的根文件名
    // 在java源码当中，不要将应用的根文件名写死，因为你永远都不知道这个应用在最终部署的时候，起一个什么名字。
  ...
  - ```java
    // 获取文件的绝对路径（真实路径）
    //path为文件名（index.html）或相对于根的路径（/index.html）
    //这个geng
...
public abstract class HttpServlet extends GenericServlet {
    ...
    @Override
    //这个是原始的serivce方法，还有另一个方法名一样但传入参数不一样的方法
    public void service(ServletRequest req, ServletResponse res)
        ...
        // 调用重载的service方法，而不是循环调用。
        service(request, response);
    }
    
    // 下面，这个service方法的两个参数都是带有Http的。
    // 这个service是一个模板方法。
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp)
        ...
        // 获取请求方式
        // 这个请求方式最终可能是：""
        // 注意：request.getMethod()方法获取的是请求方式，可能是下面七种之一：
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        throws ServletException, IOException{
        // 如果不重载doGet(),而前端发以Get形式发给后端，就会报405错误
        String msg = lStrings.getString("http.method_get_not_supported");
        sendMethodNotAllowed(req, resp, msg);//405错误
    }
    
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        throws ServletException, IOException {
        // // 如果不重载dopost(),而前端发以post形式发给后端，就会报405错误
...
通过以上源代码分析：
	...
	会发生什么呢？
		发生405这样的一个错误。
		执行 sendMethodNotAllowed(req, resp, msg)；语句
...
- 如果欢迎界面在webapp的根下新建一个目录，目录中再给一个欢迎界面，那么这个欢迎页该如何设置呢？
...
- 欢迎页可以是一个Servlet吗？
  ...
  - 步骤：
    ...
    - 第三步：在web.xml文件中配置欢迎页
      ...
      - ```xml
            <welcome-file-list>
                <!-->这里的路径是上面的路径，开头/要删掉，这样就可以指向对应的Servletz<-->
    ...
    - request对象实际上又称为“请求域”对象。
      ...
      - 跳转
        ...
        - 注意
          ...
          - 在这里为什么不直接new对象然后传参数过去呢？
      
            - ```java
                  // 在AServlet当中new一个BServlet对象，然后调用BServlet对象的doGet方法，把request对象传过去。
                  // 这个代码虽然可以实现功能，但是Servlet对象不能自己由程序员来new。自己new的Servlet对象生命周期不受Tomcat服务器的管理，是个非法对象。
      ...
      - 转发 的下一个资源必须是一个Servlet吗？
      
        - 不一定，只要是Tomcat服务器当中的合法资源，都是可以转发的。例如：（前端）html....
      ...
      - HttpServletRequest接口的其他常用方法：
        ...
        - ```java
          ...
          // Tomcat9前（包括9在内），如果前端[请求体]提交的是中文，后端获取之后出现乱码，怎么解决这个乱码？执行以下代码。
          request.setCharacterEncoding("UTF-8");
          
          
          // get请求乱码问题怎么解决？
          // get请求发送的时候，数据是在请求行上提交的，不是在请求体当中提交的。
          // 方案：修改CATALINA_HOME/conf/server.xml配置文件，添加URIEncoding属性
          <Connector URIEncoding="UTF-8" />
          // 注意：从Tomcat8之后，URIEncoding的默认值就是UTF-8，所以GET请求也没有乱码问题了。
          
          
          // 在Tomcat9之前（包括9），[响应]中文也是有乱码的，怎么解决这个响应的乱码？
          ...
          // 获取请求的URI
          String uri = request.getRequestURI();  // /aaa/testRequest，/项目名/path
  ...
  - 代码上有什么区别？
    ...
    - 重定向
      ...
      - ```java
        ...
        //response.sendRedirect(requset.getContextPath+"/dept/list");
        ```
...
- 转发和重定向应该如何选择？什么时候使用转发，什么时候使用重定向？
  ...
  - 剩下所有的请求均使用重定向。（重定向使用较多,如果两地距离过长，会卡一些，因为要反复请求。）

- 重定向跳转的下一个资源有没有要求呢？必须是一个Servlet吗？

  - 不一定，重定向跳转的资源只要是服务器内部合法的资源即可。包括：Servlet、JSP、HTML.....

- 转发会存在浏览器的刷新问题。

  - **原因：**请求转发是不改变浏览器的URL路径的，URL不改变浏览器的里面的表单数据也还在，刷新就是重新提交一次
  - **解决：**用重定向的响应方式后浏览器就不会再保存原来URL的表单了
...
- 我们的第一个注解：
  ...
  - 在Servlet类上使用：@WebServlet()，WebServlet注解中有哪些属性呢？
    ...
    - value属性：当注解的属性名只有value的时候，使用注解的时候，value属性名是可以省略的。
...
- 注解和xml的使用规范
  ...
  - 注解：最大的好处就是简化了XML配置；但是分散在各处不好维护，所以在不经常改变的时候使用(可以用工具辅助管理)
  - XML：配置集中化管理，方便修改
...
- 为什么不使用request对象保存会话状态？为什么不使用ServletContext对象保存会话状态？
  ...
  - ServletContext对象的域太大，生命周期太长。
  - request请求域（HttpServletRequest）、session会话域（HttpSession）、application应用域（ServletContext）
  - request < session < application

![session对象](D:\大学资料\学习资料\Java后端\JavaWeb\tomcat&servlet\老杜2021最新JavaWeb\文档\select.assets\session对象.png)
...
- session的实现原理：
  ...
  - Set-Cookie：JSESSIONID=(32位)  这个是以Cookie的形式保存在浏览器的内存中的。浏览器只要关闭。这个cookie就没有了。
...
- Cookie禁用了，session还能找到吗？
  ...
  - 浏览器cookie找不到jsessionid了。每一次请求都会获取到新的session对象。
  - cookie禁用了，session机制还能实现吗？
    - 可以。需要使用URL重写机制。
    - http://localhost:8080/servlet12/test/session;jsessionid=19D1C99560DCBF84839FA43D58F56E16
    - URL路径;jsessionid=32位码
    - 在实际上是不用这个方法的，因为URL重写机制会提高开发者的成本。开发人员在编写任何请求路径的时候，后面都要添加一个sessionid，给开发带来了很大的难度，很大的成本。
    - 所以大部分的网站都是，只要你要是禁用cookie，你就无法访问网站内容。

- 在jsp里禁止生成session对象，在jsp对应的java代码里，会自动调用getsession();

- 所以jsp本身会创建一个新的sessionid
...
- 关于cookie的path，cookie关联的路径：
  ...
  - 从浏览器发给服务器的时候，cookie在什么情况下会随着request一起发给服务器？
  - 假设现在发送的请求路径是“http://localhost:8080/servlet13/cookie/generate”生成的cookie，如果cookie没有设置path，默认的path是什么？
    ...
    - 也就是说，以后只要浏览器的请求路径是http://localhost:8080/servlet13/cookie 以这个路径开头的所用子路径，cookie都会被发送到服务器。
  - 手动设置cookie的path
    - 在服务器里加上，cookie.setPath(“/servlet13”); 发给浏览器后，表示只要是这个浏览器servlet13项目的请求路径，都会提交这个cookie给服务器。
...
- 通过EL表达式获取本地cookie的值：
  ...
  - ${cookie.目标名字.属性} //属性可以是 name（目标名），value（值），maxAge（存在时间、秒）等等
...
- JSP的基础语法
  ...
  - JSP的page指令（这个指令后面再详细说，这里先解决一下中文乱码问题），解决响应时的中文乱码问题：
    ...
    - 通过page指令来设置响应的内容类型，在内容类型的最后面添加：charset=UTF-8
      ...
      - <%@page import="java.util.List,java.util.ArrayList"%> ，导入包名，在用类的时候记得加，快捷键alt+回车也可以自动导入
  ...
  - 在JSP中如何编写JSP的专业注释
    ...
    - <!--这种注释属于HTML的注释，这个注释信息仍然会被jsp翻译到java源代码当中，不建议。-->
  ...
  - JSP基础语法总结：
    ...
    - <%! %>
      - 翻译到service方法之外。
    - <%= %>
      - 翻译到service方法体内部，翻译为：out.print();  不是字符串
...
- EL表达式的使用：
  ...
  - EL表达式取数据的时候有两种形式：
    ...
    - 第一种：” . “  （大部分使用这种方式）
      - 也可以${sessionScope.a}在规定的作用域里读数据
